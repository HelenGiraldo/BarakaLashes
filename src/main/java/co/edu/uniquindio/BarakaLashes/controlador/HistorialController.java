...
public class HistorialController {
    ...
    public String mostrarHistorial(Model model, HttpSession session) {
        try {
            // Obtener email de la sesión (tu AuthController lo guarda como "usuarioEmail" o "email")
            String email = (String) session.getAttribute("usuarioEmail");

            // Si no está con ese nombre, intentar con "email"
            ...
            if (email == null || email.isEmpty()) {
                log.warn("⚠️ Usuario no autenticado intentando acceder al historial");
            ...
            if (!citas.isEmpty()) {
                ...
                log.info("ℹ️ No se encontraron citas para el usuario: {}", email);
            }
    ...
    public String cancelarCita(@PathVariable Integer id,
        ...
        try {
            ...
            // Verificar autenticación
            if (email == null || email.isEmpty()) {
                log.warn("⚠️ Usuario no autenticado intentando cancelar cita");
